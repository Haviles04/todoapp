...
function App() {
  ...
  const [todoList, setTodoList] = useState(() => {
    ...
    const parsedSavedTodo = JSON.parse(savedTodoList);
    if (parsedSavedTodo.length < 1 || JSON.stringify(savedTodoList) === JSON.stringify(initalTodoList)) {
      return initalTodoList;
    } else {
      return parsedSavedTodo
    }
  });
