...
function TaskList(props) {
  ...
  function clearComplete() {
    let completedArray = [...props.todoList].filter((todo) => !todo.complete);
    props.setTodoList(completedArray);
  }

  //Only shows completed tasks within todolist
  function displayComplete(){
    props.todoList.forEach((todo) => {
      if(!todo.complete){
        document.getElementById(`div ${todo.id}`).style.display="hidden";
      }
    })
  }
  ...
  function displayAll(){
    props.todoList.forEach((todo)=>{
      document.getElementById(`div ${todo.id}`).style.display="visible";
    })
  }
