...
function TaskList(props) {
 ...
 /* function displayCompleteList() {
  ...
  const allList = props.todoList.map((todo) => (
    <TaskItem todo={todo} completeTask={completeTask} />
  ));

  const completedList = props.todoList.filter( (todo) => todo.complete).map((todo) => (
    <TaskItem todo={todo} completeTask={completeTask} />
  ));

  
  const activeList = () => props.todoList.filter( (todo) => !todo.complete).map((todo) => (
    <TaskItem todo={todo} completeTask={completeTask} />
  ));
